...
<pre>
  <code>
    ...
    Rda.configure do
      nginx_conf_paths ['/etc/nginx', '/opt/nginx/conf',
...
Please make sure that you have the write permission, or you can run:
...
If there are more than one paths found, it will give you a choice to decide
which one will be used. After choosing a proper path, it will try to
create two directories sites-available and sites-enabled to save the
